...
#                    help='the dataset to run (default: weibo)')
...
class myDataset(torch.utils.data.Dataset):
    def __init__(self,dataset):
        ...
        with open(dataset,'r',encoding='utf-8') as rfp:
            ...
            bth_sz = 32
            for idx in tqdm(range(0,len(self.data),bth_sz)):
                bth_text = self.data[idx:idx+bth_sz]
                lbth = len(bth_text)
                tmpbth = []
...
import pickle
if os.path.exists('{}_train.pkl'.format(taskname)):
    train_dataset = pickle.load(open('{}_train.pkl'.format(taskname),'rb'))
else:
    train_dataset = myDataset('{}_train.txt'.format(taskname))
    pickle.dump(train_dataset,open('{}_train.pkl'.format(taskname),'wb'))

if os.path.exists('{}_test.pkl'.format(taskname)):
    test_dataset = pickle.load(open('{}_test.pkl'.format(taskname),'rb'))
else:
    test_dataset = myDataset('{}_test.txt'.format(taskname))
    pickle.dump(test_dataset,open('{}_test.pkl'.format(taskname),'wb'))

if os.path.exists('{}_dev.pkl'.format(taskname)):
    val_dataset = pickle.load(open('{}_dev.pkl'.format(taskname),'rb'))
else:
    val_dataset = myDataset('{}_dev.txt'.format(taskname))
    pickle.dump(val_dataset,open('{}_dev.pkl'.format(taskname),'wb'))


def evaluate(isDev=True,epoch_idx=1):
    logger = open('{}_record.log'.format(taskname),'a',encoding='utf-8')
    model.eval()
    total_loss = 0.0
    count = 0
    bth_sz = 32
...
if __name__ == "__main__":
    ...
    print('test_score_df:')
    print(test_score_df)
    test_score_df.to_csv(open('{}_test_score.tsv'.format(taskname),'a'),sep='\t',index=None,header=None)
